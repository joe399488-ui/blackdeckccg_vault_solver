...
const getOffsetFromEdge = (edge, w, h) => {
  ...
  const offsets = {
    ...
    // 1x4 Specifics
    'middle-left': { dr: 0, dc: -1 }, // For H: index 1
    'middle-right': { dr: 0, dc: -2 }, // For H: index 2
    'middle-top': { dr: -1, dc: 0 },  // For V
    'middle-bottom': { dr: -2, dc: 0 } // For V
  };

  return offsets[edge] || { dr: 0, dc: 0 };
};

export default function DiamondBattleshipSolver() {
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto font-sans">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Diamond Solver</h1>
